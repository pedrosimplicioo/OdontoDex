...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (authUser.emailVerified !== true) {
      return res.status(403).json({ ok: false, status: "email_not_verified", error: "Email nao verificado" });
    }
    ...
    if (result.status === "profile_missing") {
      return res.status(409).json({ ok: false, status: result.status, error: "Perfil do usuario ainda nao foi criado" });
    }
